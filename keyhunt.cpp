...
void menu();
void init_generator();
void print_bsgs_dance_progress();
...
void print_bsgs_dance_progress() {
	...
	if(FLAGMODE != MODE_BSGS || FLAGBSGSMODE != BSGS_MODE_DANCE || FLAGRANGE == 0) {
		return;
	}

	low = BSGS_CURRENT.GetBase16();
	high = n_range_end.GetBase16();
	if(n_range_end.IsGreater(&BSGS_CURRENT)) {
		remaining_range.Set(&n_range_end);
		remaining_range.Sub(&BSGS_CURRENT);
	}
	else {
		remaining_range.SetInt32(0);
	}
	remaining = remaining_range.GetBase16();

	printf("\n[+] BSGS_PROGRESS mode=dance low=0x%s high=0x%s remaining=0x%s\n",low,high,remaining);
	fflush(stdout);
